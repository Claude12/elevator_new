...
<?php if ( $has_orders ) : ?>
	...
	<!-- Custom: client-side order search -->
	<input type="text" id="order-search" class="wooacc-search"
		placeholder="<?php esc_attr_e( 'Search orders...', 'elevator' ); ?>">
		...
		<tbody>
			...
			foreach ( $customer_orders->orders as $customer_order ) {
				...
				<tr class="woocommerce-orders-table__row woocommerce-orders-table__row--status-<?php echo esc_attr( $order->get_status() ); ?> order">
					...
					<?php
					// Custom: hidden cell with product names for search functionality.
					$product_names = array_map( function ( $item ) {
						return $item->get_name();
					}, $order->get_items() );
					?>
					<td class="order-products" style="display:none;">
						<?php echo esc_html( implode( ', ', $product_names ) ); ?>
					</td>
				</tr>
				<?php
			}
	...
	<!-- Custom: no-match message for order search -->
	<p id="no-orders-message" style="display:none; text-align:center; margin-top:20px;">
		<?php esc_html_e( 'No matching orders found.', 'elevator' ); ?>
	</p>

	<script>
	document.addEventListener( 'DOMContentLoaded', function () {
		var searchInput = document.getElementById( 'order-search' );
		var rows        = document.querySelectorAll( '.woocommerce-orders-table tbody tr' );
		var noOrdersMsg = document.getElementById( 'no-orders-message' );
		...
		searchInput.addEventListener( 'keyup', function () {
			...
			noOrdersMsg.style.display = anyVisible ? 'none' : 'block';
		});
	});
	</script>
...
<?php else : ?>
	...
	<?php
	wc_print_notice(
		esc_html__( 'No order has been made yet.', 'woocommerce' ) . ' <a class="woocommerce-Button wc-forward button' . esc_attr( $wp_button_class ) . '" href="' . esc_url( apply_filters( 'woocommerce_return_to_shop_redirect', wc_get_page_permalink( 'shop' ) ) ) . '">' . esc_html__( 'Browse products', 'woocommerce' ) . '</a>',
		'notice'
	);
	?>

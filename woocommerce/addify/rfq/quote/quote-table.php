<?php
/**
 * Customer information table for PDF.
 * CUSTOMIZED V3: Reorganized pricing columns and added discount column
 *
		...
		<thead>
			<tr>
				...
				<?php if ( $price_display ) : ?>
					...
					<th class="product-discount"><?php esc_html_e( 'Discount %', 'addify_b2b' ); ?></th>
				<?php endif; ?>
				<th class="product-quantity"><?php esc_html_e( 'Quantity', 'addify_b2b' ); ?></th>
				<?php if ( $price_display ) : ?>
					<th class="product-net-value"><?php esc_html_e( 'Net Value', 'addify_b2b' ); ?></th>
				<?php endif; ?>
				<?php if ( $of_price_display ) : ?>
					<th class="product-offered-price"><?php esc_html_e( 'Offered Price', 'addify_b2b' ); ?></th>
					<th class="product-offered-net-value"><?php esc_html_e( 'Offered Net Value', 'addify_b2b' ); ?></th>
				<?php endif; ?>
			</tr>
		</thead>
		<tbody>
					...
					<tr class="<?php echo esc_attr( apply_filters( 'addify_quote_item_class', 'cart_item', $quote_item, $quote_item_key ) ); ?>">
						...
						<td class="product-name" data-title="<?php esc_attr_e( 'Product', 'addify_b2b' ); ?>">
							...
							if ( $_product->backorders_require_notification() && $_product->is_on_backorder( $quote_item['quantity'] ) ) {
								echo wp_kses_post( apply_filters( 'addify_quote_item_backorder_notification', '<p class="backorder_notification">' . esc_html__( 'Available on backorder', 'addify_b2b' ) . '</p>', $quote_item, $quote_item_key ) );
							}
						...
						<?php if ( $price_display ) : ?>
							<td class="product-price" data-title="<?php esc_attr_e( 'Price', 'addify_b2b' ); ?>">
								<?php
									...
									echo wp_kses_post( apply_filters( 'addify_quote_item_price', $af_quote->get_product_price( $_product, $args ), $quote_item, $quote_item_key ) );
								?>
							</td>
							<td class="product-discount" data-title="<?php esc_attr_e( 'Discount %', 'addify_b2b' ); ?>">
								<?php
									echo esc_html( $discount_percentage > 0 ? $discount_percentage . '%' : '-' );
								?>
							</td>
						<?php endif; ?>
						...
						<?php if ( $price_display && ( !isset($quote_item['composite_child_products']) || empty($quote_item['composite_child_products']) ) ) : ?>
							<td class="product-net-value" data-title="<?php esc_attr_e( 'Net Value', 'addify_b2b' ); ?>">
								<?php
									...
									$args['price'] = empty( $quote_item['price_calculator_price'] ) ? $args['price'] : $quote_item['price_calculator_price'];
									$args['price'] = isset( $quote_item['composite_product_price'] ) ? $quote_item['composite_product_price'] : $args['price'];
									echo wp_kses_post( apply_filters( 'addify_quote_item_net_value', $af_quote->get_product_subtotal( $_product, $quote_item['quantity'], $args ), $quote_item, $quote_item_key ) );
								?>
							</td>
						<?php endif; ?>	

						<?php if ( $of_price_display && ( !isset($quote_item['composite_child_products']) || empty($quote_item['composite_child_products']) ) ) : ?>
							<td class="product-offered-price" data-title="<?php esc_attr_e( 'Offered Price', 'addify_b2b' ); ?>">
							...
							<td class="product-offered-net-value" data-title="<?php esc_attr_e( 'Offered Net Value', 'addify_b2b' ); ?>">
								<?php
									echo wp_kses_post( apply_filters( 'addify_quote_item_offered_net_value', wc_price( $offered_price * $quote_item['quantity'] ), $quote_item, $quote_item_key ) );
								?>
							</td>
						<?php endif; ?>
